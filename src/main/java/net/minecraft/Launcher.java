...
public class Launcher extends Applet implements AppletStub {
	...
	public Launcher(File instance, File lwjgl, String username, String sessionid, String host, String port, boolean doConnect) {
		...
		URLClassLoader classLoader = new URLClassLoader(urls, Launcher.class.getClassLoader());
		try {
	...
	private Field findPathField(Class<?> minecraft) {
		...
		for (Field current : fields) {
			if (current.getType() != File.class || (current.getModifiers() != (Modifier.PRIVATE + Modifier.STATIC))) {
				continue;
			}
			return current;
		}
